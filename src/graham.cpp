...
bool cmp(const point& i, const point& j){
    int l = PIVO.left(i, j);
    if(l > 0 || (l == 0 && PIVO.dist(i) < PIVO.dist(j))) {
        return true;
    }
    return false;
}
...
void graham(struct point* P, int N, struct point* CH, int* CH_N) {
    int pivo = 0;
    for(int i = 1; i < N; i++) {
        if(P[i].y < P[pivo].y || (P[i].y == P[pivo].y && P[i].x < P[pivo].x)) {
            pivo = i;
        }
    }
    swap(P[pivo], P[0]);
    ...
    *CH_N = 0;
    for(int i = 0; i < N; i++) {
        if(i > 1) {
            ...
            while(*CH_N >= 2 && CH[*CH_N-2].left(CH[*CH_N-1],P[i]) <= 0) {
                (*CH_N)--;
            }
        }
        CH[(*CH_N)++] = P[i];
...
bool pointInLine(point a, point b, point c) {
    if(b.y != c.y) {
        if(min(b.y,c.y) <= a.y && a.y <= max(b.y, c.y)) {
            return true;
        }
        return false;
    }
